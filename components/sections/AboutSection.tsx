...
const AboutSection: React.FC = () => {
    ...
    // teamFeatures will be translated directly in the JSX for simplicity with dynamic keys
    const teamFeatures = [
        {
            key: 'feature1',
            icon: Brain,
            titleDefault: "מומחיות פדגוגית",
            descriptionDefault: "צוות המורכב ממורים מנוסים, מומחים למחוננים ופסיכולוגים חינוכיים.",
            delay: 0.5
        },
        {
            key: 'feature2',
            icon: CheckCircle,
            titleDefault: "תוכן מותאם ועדכני",
            descriptionDefault: "קורסים דיגיטליים חדשניים עם תרגולים אינטראקטיביים ופתרונות מוסברים.",
            delay: 0.65
        },
        {
            key: 'feature3',
            icon: Star,
            titleDefault: "ליווי אישי להצלחה",
            descriptionDefault: "מעקב התקדמות, מבחני דמה, וליווי להורים לאורך כל הדרך.",
            delay: 0.8
        }
    ];

    // ABOUT_STATS labels will be translated directly in the JSX
    // TEAM_MEMBERS_DATA roles will be translated directly in the JSX for roles that require it. Names are proper nouns.
        ...
        <section className="py-16 sm:py-20 md:py-24 bg-white dark:bg-secondary">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <AnimatedDiv animation="fadeInUp" className="text-center mb-12 sm:mb-16">
                    <h2 className="text-lg font-semibold text-primary tracking-wider uppercase">{t('aboutSection.whoWeAre', 'מי אנחנו')}</h2>
                    <p className="mt-2 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-white leading-tight">
                        {t('aboutSection.mainTitle', `${APP_NAME}: מכינים את ילדיכם למצוינות והצלחה`, { appName: t('appName', APP_NAME) })}
                    </p>
                </AnimatedDiv>
                ...
                <div className="max-w-4xl mx-auto text-center mb-12 sm:mb-16">
                    <AnimatedDiv animation="fadeInUp" delay={0.1}>
                        <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100 mb-4 sm:mb-6">
                            {t('aboutSection.leadingSiteTitle', 'האתר המוביל בישראל להכנה למבחני מחוננים ותוכניות ייחודיות')}
                        </h3>
                        <p className="text-base sm:text-lg text-gray-700 dark:text-gray-300 leading-relaxed sm:leading-loose mb-4">
                            {t('aboutSection.description1', `אנו ב${APP_NAME} מתמחים בפיתוח קורסים דיגיטליים חדשניים, המותאמים במיוחד לילדים בכיתות א’ עד ו’. מטרתנו היא להעניק לכל ילד את הכלים הטובים ביותר להתמודדות עם מבחני המחוננים, מבחני קבלה לבתי ספר יוקרתיים, תוכניות העשרה וכיתות מצוינות.`, { appName: t('appName', APP_NAME) })}
                        </p>
                        <p className="text-base sm:text-lg text-gray-700 dark:text-gray-300 leading-relaxed sm:leading-loose">
                            {t('aboutSection.description2', 'הקורסים שלנו משלבים סרטונים אינטראקטיביים, תרגולים מותאמים אישית, פתרונות מוסברים, וטיפים חיוניים להתמודדות עם לחץ, אסטרטגיות פתרון וחשיבה יצירתית. אנו שואפים לא רק להכין למבחן, אלא גם לפתח ביטחון עצמי, יכולת חשיבה עצמאית ואהבה ללמידה.')}
                        </p>
                    </AnimatedDiv>
                </div>
                    ...
                    {teamFeatures.map(feature => (
                        ...
                        >
                            ...
                            <h4 className="text-lg sm:text-xl lg:text-2xl font-semibold text-gray-800 dark:text-white mb-3 transition-colors duration-300 group-hover:text-primary dark:group-hover:text-primary-light text-center">
                                {t(`aboutSection.${feature.key}.title`, feature.titleDefault)}
                            </h4>
                            <p className="text-gray-600 dark:text-gray-400 leading-relaxed text-sm sm:text-base text-center">
                                {t(`aboutSection.${feature.key}.description`, feature.descriptionDefault)}
                            </p>
                        </AnimatedDiv>
                    ))}
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 text-center">
                        {ABOUT_STATS.map((stat, index) => (
                            <AnimatedDiv key={stat.label} animation="zoomIn" delay={0.4 + index * 0.1} className="p-4 bg-gray-100 dark:bg-secondary-light rounded-lg shadow-md">
                                ...
                                <div className="text-sm sm:text-base text-gray-600 dark:text-gray-400">{t(`aboutSection.stats.${stat.label.toLowerCase().replace(/\s+/g, '')}`, stat.label)}</div>
                            </AnimatedDiv>
                        ))}
                    </div>
                </AnimatedDiv>
                ...
                <AnimatedDiv animation="fadeInUp" delay={0.5} className="mb-16 sm:mb-20">
                    <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white text-center mb-10 sm:mb-12">{t('aboutSection.ourExpertsTitle', 'צוות המומחים שלנו')}</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        {TEAM_MEMBERS_DATA.map((member, index) => (
                            <AnimatedDiv key={index} animation="fadeInUp" delay={0.6 + index * 0.1} className="text-center p-6 bg-gray-50 dark:bg-secondary-light rounded-xl shadow-lg hover:shadow-xl transition-shadow">
                                ...
                                {/* Role translation is complex due to split. Assuming roles are fixed or translated in constants.tsx if needed */}
                                <p className="text-sm text-primary dark:text-primary-light mb-2">{t(`teamRoles.${member.role.split(' ')[0].toLowerCase()}`, member.role.split(' ')[0])}</p>
                                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">{t(`teamRoles.${member.role.substring(member.role.indexOf(' ') + 1).toLowerCase().replace(/\s+/g, '')}`, member.role.substring(member.role.indexOf(' ') + 1))}</p>
                            </AnimatedDiv>
                        ))}
                    </div>
                </AnimatedDiv>
                ...
                <AnimatedDiv animation="fadeInUp" delay={0.7} className="mb-12 sm:mb-16">
                    <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white text-center mb-10 sm:mb-12">{t('aboutSection.testimonialsTitle', 'מה לקוחותינו מספרים')}</h3>
                    <div className="max-w-3xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
                        {[1, 2].map(i => ( // Assuming 2 static testimonials for now
                            <div key={i} className="p-6 bg-gray-50 dark:bg-secondary-light rounded-xl shadow-lg">
                                ...
                                <p className="text-gray-700 dark:text-gray-300 italic mb-3 leading-relaxed">
                                    {t(`aboutSection.testimonial${i}.text`, "הקורס היה מעולה! הבן שלנו הגיע מוכן למבחן והצליח מאוד. התרגול האינטראקטיבי וההסברים היו ברורים מאוד. ממליצים בחום!")}
                                </p>
                                <p className="font-semibold text-gray-800 dark:text-white">{t(`aboutSection.testimonial${i}.author`, "- משפחת כהן (שם בדוי)")}</p>
                            </div>
                        ))}
                    </div>
                </AnimatedDiv>
                ...
                <AnimatedDiv animation="fadeInUp" delay={0.8} className="text-center">
                    <Button onClick={handleDiscoverCoursesClick} size="lg" variant="primary" className="text-lg sm:text-xl px-8 py-3.5">
                        {t('aboutSection.joinSuccessButton', 'הצטרפו להצלחה - גלו את הקורסים שלנו')}
                    </Button>
                </AnimatedDiv>
            </div>
        </section>
