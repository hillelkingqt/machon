    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response.ok && result.success) {
                ...
                setFeedbackMessage(t('contactSection.formBox.feedbackSuccess', 'ההודעה נשלחה בהצלחה! ניצור איתך קשר בהקדם.'));
                setFormData({ name: '', email: '', message: '' }); // Reset form
            } else {
                setSubmissionStatus('error');
                setFeedbackMessage(result.error || t('contactSection.formBox.feedbackErrorDefault', 'שליחת ההודעה נכשלה. אנא נסו שוב מאוחר יותר.'));
                console.error('Submission error:', result.error || 'Unknown error');
            }
        } catch (error) {
            setSubmissionStatus('error');
            setFeedbackMessage(t('contactSection.formBox.feedbackErrorUnexpected', 'אירעה שגיאה בלתי צפויה. אנא נסו שוב מאוחר יותר.'));
        ...
        <section className="py-16 sm:py-20 md:py-24 bg-white dark:bg-secondary">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <AnimatedDiv animation="fadeInUp" className="text-center mb-12 sm:mb-16">
                    <h2 className="text-lg font-semibold text-primary tracking-wider uppercase">{t('contactSection.titlePart1', 'צור קשר')}</h2>
                    <p className="mt-2 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-900 dark:text-white leading-tight">
                        {t('contactSection.titlePart2', 'אנחנו כאן לשירותכם')}
                    </p>
                    <p className="mt-4 max-w-2xl mx-auto text-base sm:text-lg md:text-xl text-gray-600 dark:text-gray-400 leading-relaxed">
                        {t('contactSection.description', 'יש לכם שאלות? רוצים להתייעץ? אל תהססו לפנות אלינו. נשמח לעמוד לרשותכם בכל עניין.')}
                    </p>
                </AnimatedDiv>
                ...
                <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-start">
                    ...
                    >
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-6">{t('contactSection.detailsBox.title', 'פרטי התקשרות')}</h3>
                        <div className="space-y-4 sm:space-y-5 text-base sm:text-lg">
                            <a href={CONTACT_DETAILS.mapLink} target="_blank" rel="noopener noreferrer" className="flex items-start text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary-light transition-colors group">
                                ...
                                <span className="flex-1">{t('footer.address', CONTACT_DETAILS.address)}</span>
                            ...
                            <a href={CONTACT_DETAILS.phoneLink} className="flex items-center text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary-light transition-colors group">
                                ...
                                <span>{t('footer.phone', CONTACT_DETAILS.phone)}</span>
                            </a>
                            <a href={`mailto:${CONTACT_DETAILS.email}`} className="flex items-center text-gray-700 dark:text-gray-300 hover:text-primary dark:hover:text-primary-light transition-colors group">
                                ...
                                <span>{t('footer.email', CONTACT_DETAILS.email)}</span>
                            </a>
                        </div>
                        <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700/50">
                            <Button
                                href={`https://wa.me/${getWhatsAppNumber(CONTACT_DETAILS.phone)}?text=${encodeURIComponent(t('contactSection.detailsBox.whatsappMessage', "שלום, הגעתי מאתר מכון אביב ואשמח לקבל פרטים נוספים."))}`}
                                external
                                variant="success"
                            ...
                            >
                                {t('contactSection.detailsBox.whatsappButton', 'שליחת הודעה בוואטסאפ')}
                            </Button>
                        </div>
                    </AnimatedDiv>
                    ...
                    <AnimatedDiv
                        ...
                        className="bg-gray-50 dark:bg-secondary-light p-6 sm:p-8 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1.5"
                    >
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-6">{t('contactSection.formBox.title', 'שלחו לנו הודעה')}</h3>
                        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('contactSection.formBox.nameLabel', 'שם מלא')}<span className="text-red-500 ms-1">*</span></label>
                                <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} required disabled={submissionStatus === 'loading'} className="w-full px-4 py-2.5 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-primary focus:border-primary dark:bg-secondary dark:text-white transition-all duration-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 disabled:opacity-70 disabled:bg-gray-200 dark:disabled:bg-gray-700" placeholder={t('contactSection.formBox.namePlaceholder', 'לדוגמה: ישראל ישראלי')} />
                            </div>
                            <div>
                                <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('contactSection.formBox.emailLabel', 'אימייל')}<span className="text-red-500 ms-1">*</span></label>
                                <input type="email" name="email" id="email" value={formData.email} onChange={handleInputChange} required disabled={submissionStatus === 'loading'} className="w-full px-4 py-2.5 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-primary focus:border-primary dark:bg-secondary dark:text-white transition-all duration-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 disabled:opacity-70 disabled:bg-gray-200 dark:disabled:bg-gray-700" placeholder={t('contactSection.formBox.emailPlaceholder', 'your@email.com')} />
                            </div>
                            <div>
                                <label htmlFor="message" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('contactSection.formBox.messageLabel', 'הודעה')}<span className="text-red-500 ms-1">*</span></label>
                                <textarea name="message" id="message" value={formData.message} onChange={handleInputChange} rows={4} required disabled={submissionStatus === 'loading'} className="w-full px-4 py-2.5 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-primary focus:border-primary dark:bg-secondary dark:text-white transition-all duration-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 disabled:opacity-70 disabled:bg-gray-200 dark:disabled:bg-gray-700" placeholder={t('contactSection.formBox.messagePlaceholder', 'כתבו את פנייתכם כאן...')} ></textarea>
                            </div>
                            <div>
                                <Button type="submit" variant="primary" className="w-full text-base sm:text-lg py-3 shadow-lg hover:shadow-xl" icon={submissionStatus === 'loading' ? <Loader2 size={20} className="animate-spin" /> : <Send size={18} />} iconPosition="trailing" disabled={submissionStatus === 'loading'}>
                                    {submissionStatus === 'loading' ? t('contactSection.formBox.loadingButton', 'שולח...') : t('contactSection.formBox.submitButton', 'שליחה')}
                                </Button>
                            </div>
                            {feedbackMessage && (
